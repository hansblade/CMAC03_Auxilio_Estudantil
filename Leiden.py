...
# ==============================================
# 5. Executar Algoritmo Leiden
# ==============================================
def rodar_leiden(g: ig.Graph, weights: list) -> dict:
    """
    Executa o método Leiden no grafo g, atribui cores conforme comunidades,
    ...
    palette_leiden = ig.drawing.colors.ClusterColoringPalette(len(partition))
    g_leiden.vs['color'] = [palette_leiden[c] for c in partition.membership]

    caminho_leiden = tempfile.mktemp(suffix="_leiden.png")
    ig.plot(
        g_leiden,
        layout=layout,
        vertex_size=20,
        ...
        bbox=(800, 800),
        margin=40,
        target=caminho_leiden
    )

    return {
        'num_comunidades': len(partition),
        'modularidade': partition.modularity,
        'caminho_imagem': caminho_leiden,
        'membership': partition.membership
    }


# ==============================================
# 6. Calcular Estatísticas por Grupo Leiden
# ==============================================
def calcular_estatisticas_por_grupo(df: pd.DataFrame, membership: list) -> (pd.DataFrame, pd.DataFrame):
    ...
    Devolve:
    - df_annotated: DataFrame com uma coluna adicional 'Grupo Leiden'.
    - stats_por_grupo: DataFrame contendo colunas ['Grupo', 'Media Vulnerabilidade', 'Desvio Padrão', 'N de Estudantes'].
    """
    df_annotated = df.copy()
    df_annotated['Grupo Leiden'] = membership

    # Agrupar e computar estatísticas
    agrup = df_annotated.groupby('Grupo Leiden')['Indice Vulnerabilidade']
...
# ==============================================
# 7. Gerar Histograma de Médias por Grupo Leiden
# ==============================================
...
class PDF(FPDF):
    def header(self):
        ...
        self.cell(0, 10, "Análise de Vulnerabilidade Estudantil - 2018 (Leiden)", ln=True, align='C')
        self.ln(4)
...
# ==============================================
# 9. Gerar Relatório em PDF (Leiden)
# ==============================================
def gerar_relatorio_pdf_leiden(
    resultado_leiden: dict,
    ...
    """
    Recebe:
    - resultado_leiden: Saída de rodar_leiden()
    - df_annotated: DataFrame com coluna 'Indice Vulnerabilidade' e 'Grupo Leiden'
    - stats_por_grupo: DataFrame de estatísticas por grupo Leiden
    ...
    # -- Seção 1: Resumo do Algoritmo Leiden --
    pdf.chapter_title("1. Resumo do Algoritmo Leiden")
    ld = resultado_leiden
    texto_resumo = (
        f"Algoritmo Leiden:\n"
        f"  - Número de Comunidades: {ld['num_comunidades']}\n"
        f"  - Modularidade: {ld['modularidade']:.4f}\n"
    )
    pdf.chapter_body(texto_resumo)

    # -- Seção 2: Gráfico Leiden --
    pdf.add_image(ld['caminho_imagem'], "2. Grafo - Leiden")

    # -- Seção 3: Histograma de Médias por Grupo Leiden --
    pdf.add_image(caminho_hist, "3. Média de Vulnerabilidade por Grupo (Leiden)")

    # -- Seção 4: Tabela detalhada de estudantes --
    pdf.chapter_title("4. Tabela de Índice de Vulnerabilidade por Estudante (com Grupo Leiden)")
    pdf.chapter_body("A seguir, listam-se os 10 estudantes com meior índice e o grupo Leiden correspondente:")

    tabela = df_annotated.copy()
    tabela = tabela[['Indice Vulnerabilidade', 'Grupo Leiden']].reset_index()
    tabela = tabela.rename(columns={'index': 'ID', 'Indice Vulnerabilidade': 'Índice', 'Grupo Leiden': 'Grupo'})
    ...
    # -- Seção 5: Estatísticas por Grupo Leiden --
    pdf.add_page()
    ...
    try:
        os.remove(ld['caminho_imagem'])
        os.remove(caminho_hist)
    except OSError:
        pass

    print(f"Relatório Leiden gerado com sucesso em: {caminho_saida}")
...
if __name__ == "__main__":
    ...
    # 10.4. Rodar algoritmo Leiden
    resultado_leiden = rodar_leiden(grafo, pesos)

    # 10.5. Calcular estatísticas por grupo (Leiden)
    df_annotated, stats_leiden = calcular_estatisticas_por_grupo(
        df_vul, resultado_leiden['membership']
    )

    # 10.6. Exportar estatísticas para CSV
    stats_leiden.to_csv(OUTPUT_CSV, index=False, encoding='utf-8-sig')
    print(f"Arquivo CSV com estatísticas Leiden salvo em: {OUTPUT_CSV}")

    # 10.7. Gerar histograma de médias por grupo (Leiden)
    caminho_hist = gerar_histograma_medias_por_grupo(stats_leiden)

    # 10.8. Gerar relatório em PDF (Leiden)
    gerar_relatorio_pdf_leiden(
        resultado_leiden,
        df_annotated,
        stats_leiden,
        caminho_hist,
